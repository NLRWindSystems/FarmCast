...
def set_turbsim(n_turbines, rotor_diameter, hub_height, ws, spacing, wind_direction, 
                Cmeander = 1.9, transient = 120, analysis_time = 600, 
                domain_edge = [1., 1., 1., 1., 1.], dy = 10., dz = 10., 
                res = 'low', TimeStep_HR=0.25, mod_wake=1):
    """
    ...
    domain_edge : list, optional
        The extra size in rotor diameters along the edges of the domain for x (left and right) and y 
        (top and bottom) and z for the TurbSim box. Default is [1., 1., 1., 1., 1.].
    dy : float, optional
    ...
    if res == 'low':
        Width = (
            ...
            + (domain_edge[2] + domain_edge[3]) * rotor_diameter
        )
        Height = hub_height  + 2 * rotor_diameter
        if mod_wake == 1:
        ...
        Width = rotor_diameter * (1. + (domain_edge[2] + domain_edge[3]))
        Height = hub_height  + 0.5 * rotor_diameter * (1. + domain_edge[4]) -1.
    ...
    AnalysisTime = np.ceil(
        (
            ...
            + (domain_edge[0] + domain_edge[1]) * rotor_diameter
        ) / ws
        + transient 
        + analysis_time
    )
